...
fun ICorChainDsl<TimetableContext>.stubSearchSuccess(title: String) = worker {
    this.title = title
    on { stubCase == TimetableStubs.SUCCESS && state == TimetableState.RUNNING }
    handle {
        state = TimetableState.FINISHING
        tripsResponse.addAll(TimetableTripStub.prepareSearchList(tripFilterRequest.searchString, TimetableDirection.FORWARD))
    }
}
